...
// Create thumbnail (only if provided)
let thumbnail = null;
if (publication.thumbnail) {
  ...
  publication.authors.forEach((author, index) => {
    if (author.includes('Vishisht Rao')) { // TODO: Highlight specific author
...
// Presentations (plain text, one per line)
if (publication.presentations && publication.presentations.length > 0) {
  const presContainer = document.createElement('div');
  presContainer.className = 'pub-presentations';

  publication.presentations.forEach(p => {
    const line = document.createElement('div'); // new line
    const label = `${p.type || 'Poster'} – ${p.venue}${p.year ? ` (${p.year})` : ''}`;
    ...
    presContainer.appendChild(line);
  });

  content.appendChild(presContainer);
}
...
if (thumbnail) pubItem.appendChild(thumbnail);
